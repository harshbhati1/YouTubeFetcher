...
//  Interface representing the structure of video information returned by yt-dlp
...
function parseUploadDate(dateStr: string): string {
...
function cleanVttTranscript(vttContent: string): string {
  ...
  const mergedSegments: { time: string; text: string }[] = [];
...
function areSegmentsOverlapping(seg1: { time: string; text: string }, seg2: { time: string; text: string }): boolean {
...
function findLongestCommonSubsequence(words1: string[], words2: string[]): string[] {
...
async function fetchVideoInfoAndTranscript(url: string): Promise<{ info: YoutubeDlInfo; transcript: string | null }> {
...
function displayVideoInfo(info: YoutubeDlInfo, transcript: string | null): void {
